...
class GameView(tk.Frame):
    ...
    def _create_widgets(self):
        ...
        for i in range(6):
            ...
            frame_line.pack()
            for j in range(5):
                lbl = tk.Label(frame_line, text="", width=2, height=1, font=FONT_GRID, bg=COLOR_BACKGROUND, fg=COLOR_LETTER, relief="solid", borderwidth=2,
                                highlightthickness=1, highlightbackground=COLOR_BORDER_ON)
    ...
    def update_grid_letter(self, line: int, column: int, letter : str = None, selected: bool = False):
        if 0 <= line < 6 and 0 <= column < 5:
            ...
            border_color = COLOR_BORDER_ON if (selected or (letter not in (""))) else COLOR_BORDER_OFF
